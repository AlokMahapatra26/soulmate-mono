...
export default function MoodLight() {
    const { showMoodLight, setShowMoodLight } = useMusic();
    ...
    }, [showMoodLight, showSettings]);

    useEffect(() => {
        if (!showMoodLight) return;
    ...
    }, [showMoodLight, currentMood, speed]);

    if (!showMoodLight) return null;
    ...
    return (
        ...
        >
            {/* Close Button */}
            <button
                onClick={() => setShowMoodLight(false)}
                style={{
                    position: 'absolute',
                    top: 24,
                    right: 24,
